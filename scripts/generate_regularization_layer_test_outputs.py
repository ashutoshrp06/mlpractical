...
parser.add_argument(
    "--student_id", nargs="?", type=str, help='Your student id in the format "Sxxxxxxx"'
)
...
for dropout_layer, params_flag in zip([DropoutLayer], [False]):
    if isinstance(dropout_layer(), DropoutLayer):
        rng = np.random.RandomState(92019)
        print(True)
        outputs = fprop_bprop_layer(
            inputs, dropout_layer(rng=rng), grads_wrt_outputs, weights, params_flag
        )
    else:
        outputs = fprop_bprop_layer(
    ...
    if params_flag:
        reg_output_dict["{}_{}".format(dropout_layer.__name__, "grads_wrt_outputs")] = (
            outputs[2]
        )

for penalty_layer, params_flag in zip([L1Penalty, L2Penalty], [False, False]):
    outputs = call_grad_layer(
        inputs, penalty_layer(1e-4), grads_wrt_outputs, weights, params_flag
    )
    reg_output_dict["{}_{}".format(penalty_layer.__name__, "__call__correct")] = (
        outputs[0]
    )
    reg_output_dict["{}_{}".format(penalty_layer.__name__, "grad_correct")] = outputs[1]
    if params_flag:
        reg_output_dict["{}_{}".format(penalty_layer.__name__, "grads_wrt_outputs")] = (
            outputs[2]
        )

np.save(
    os.path.join(
        os.environ["MLP_DATA_DIR"], "{}_regularization_test_pack.npy".format(seed)
    ),
    reg_output_dict,
)
